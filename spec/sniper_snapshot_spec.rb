...
describe SniperSnapshot do
  ...
  describe ".joining" do
    it "returns an initial snapshot" do
      expect(SniperSnapshot.joining item).to eq(
        SniperSnapshot.new(item, 0, 0, SniperState::JOINING)
      )
    end
  end

  describe "#bidding" do
    subject { SniperSnapshot.new item, 0, 0, SniperState::JOINING }
    let(:last_price) { 100 }
    let(:last_bid) { 110 }

    it "returns a new snapshot with last price and bid" do
      expect(subject.bidding last_price, last_bid ).to eq(
        SniperSnapshot.new(item, last_price, last_bid, SniperState::BIDDING)
      )
    end
  end

  describe "#winning" do
    subject { SniperSnapshot.new item, last_price, last_bid, SniperState::BIDDING }
    let(:last_price) { 100 }
    let(:last_bid) { 110 }

    it "returns a new snapshot with last price and bid" do
      expect(subject.winning last_bid ).to eq(
        SniperSnapshot.new(item, last_bid, last_bid, SniperState::WINNING)
      )
    end
  end

  describe "#won" do
    subject { SniperSnapshot.new item, last_price, last_bid, SniperState::WINNING }
    let(:last_price) { 100 }
    let(:last_bid) { 110 }
  ...
  describe "#closed" do
    ...
    context "when winning" do
      ...
      it "returns a new 'won' snapshot with last price and bid" do
        expect(subject.closed).to eq(
          SniperSnapshot.new(item, last_price, last_bid, SniperState::WON)
        )
      end
    end
    ...
    context "when not winning" do
      ...
      it "returns a new 'lost' snapshot with last price and bid" do
        expect(subject.closed).to eq(
          SniperSnapshot.new(item, last_price, last_bid, SniperState::LOST)
        )
      end
    end
  end
